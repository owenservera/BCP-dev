...
## Mainline promotion — 2026-09-25

The complete System Intelligence corpus from Pass 1–3 is now present in main under `docs/destination/system-intelligence/`. The Architecture Steward context under `AGENTS_CONTEXT/ARCHITECTURE_STEWARD/` is the standing repository role responsible for maintaining this corpus's mapping into the canonical destination architecture, dependency graph, evidence map, and current views.
