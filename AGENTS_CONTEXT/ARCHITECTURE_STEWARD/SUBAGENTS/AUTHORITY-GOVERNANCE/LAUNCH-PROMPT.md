...
**Candidate workspace:** `AGENTS_CONTEXT/ARCHITECTURE_STEWARD/SUBAGENTS/AUTHORITY-GOVERNANCE/`  
...
Read `AGENTS_CONTEXT/CORE_VS_PLUGIN_BOUNDARY/` in full, `AGENTS_CONTEXT/PERSONAL_AGENT/`, destination authority/law/consent/delegation material, Ω CURRENT-INVARIANTS and contracts concerning capability tokens, scope, revocation and grants, plus Work/Agent/Provider materials. Inspect how K0 enforcement differs from semantic authority and user policy.
...
Steward the architecture of ownership, principal identity, authority, law, consent, delegation, scope, risk and revocation so that intent never becomes permission merely through interpretation or execution.
...
Architecture of authority and its interfaces to Intent, Capability, Work, Provider, Evolution and Runtime. Do not become Ω law authority, K0 implementation owner, or a second security/secret store unless evidence proves a missing responsibility.
...
Which authority concepts deserve durable semantic records versus law syntax? Where should principal/ownership identity be owned? What belongs to law semantics versus generic K0 enforcement? Where should consent records and delegation lifecycle sit? Is risk metadata capability-owned, law-owned, or cross-cutting?
...
Before the first substantive Commons action, read the dedicated `AGENTS_CONTEXT/ARCHITECTURE_STEWARD/SUBAGENTS/AUTHORITY-GOVERNANCE/COMMUNICATION-HOW-TO.md` guide and the shared `AGENTS_CONTEXT/AGENT-COMMONS/SESSION-CAPABILITY-AND-TRANSPORT.md` contract. Determine the actual execution surface and capabilities before selecting a transport. Local full-runtime sessions should prefer `GitBranchTransport`; webapp/connector sessions should use `GitHubApiTransport` only when the correct signing key is safely available; otherwise Commons writes are read-only. Never silently mint a replacement identity.
