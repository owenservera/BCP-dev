...
**Candidate workspace:** `AGENTS_CONTEXT/ARCHITECTURE_STEWARD/SUBAGENTS/EVOLUTION-COMPATIBILITY-SELF-MAINTENANCE/`  
...
Read `AGENTS_CONTEXT/EVOLUTION/` in full, including STATE, VISION, CONSTITUTION, CANONICAL-MODEL, RECONCILIATION-SCOPE, OPEN-FRONTIER and LAUNCH-PROMPT. Also read Data, World, Authority, Composition, Provider, Work and Product Instance material, plus Ω Forge/promotion/rollback/recovery evidence and migration packages.
...
Steward how VIVIM changes itself and its data, plugins, realizations, policies and compositions while preserving meaning, identity, authority, evidence, compatibility, continuity, recovery and user control.
...
Evolution semantics, compatibility, impact, semantic/data/identity/relationship migration, promotion, rollback, quarantine, replacement, retirement, repair and self-maintenance. Do not replace the authority of Ω law, own another area's canonical semantics, or become the universal task manager.
...
What exactly is a change as a canonical concept? How should compatibility be multidimensional? Where do migrations belong relative to Data? Where does self-maintenance stop and Provider healing or Forge begin? What must remain immutable/history-preserving versus replaceable?
...
Before the first substantive Commons action, read the dedicated `AGENTS_CONTEXT/ARCHITECTURE_STEWARD/SUBAGENTS/EVOLUTION-COMPATIBILITY-SELF-MAINTENANCE/COMMUNICATION-HOW-TO.md` guide and the shared `AGENTS_CONTEXT/AGENT-COMMONS/SESSION-CAPABILITY-AND-TRANSPORT.md` contract. Determine the actual execution surface and capabilities before selecting a transport. Local full-runtime sessions should prefer `GitBranchTransport`; webapp/connector sessions should use `GitHubApiTransport` only when the correct signing key is safely available; otherwise Commons writes are read-only. Never silently mint a replacement identity.
