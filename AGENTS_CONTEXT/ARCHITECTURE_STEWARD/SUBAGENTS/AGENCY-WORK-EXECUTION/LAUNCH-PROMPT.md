...
**Candidate workspace:** `AGENTS_CONTEXT/ARCHITECTURE_STEWARD/SUBAGENTS/AGENCY-WORK-EXECUTION/`  
...
Read Personal Agent/intent material, destination Work/Agent/Execution/Background Continuity research, Product Vision journeys, Evolution recovery material, Provider/Realization context, and Ω `vivim.run`-related contracts/plugins. Inspect ActionPlan/Work lineage from the legacy mine only as evidence.
...
Steward the transformation from user intent into durable, inspectable Work and attributed execution that can recover from crashes and unknown external effects and produce truthful Outcomes and Evidence.
...
Intent-to-plan boundary where needed, durable Work, steps, attempts, execution attribution, agents, scheduler/triggers, background continuity, recovery/reconciliation and Outcome semantics. Explicitly test boundaries with Authority, Capability/Provider, Data, Evidence and Experience.
...
Where exactly does Intent end and Work begin? Is Outcome part of Work, Evidence, or a separate semantic record? Where does execution realization belong? How should unknown external effects be reconciled? How much scheduler/background state is canonical?
