...
**Core Function Area:** CFA-10 — Runtime Constitution / Core Substrate  
**Candidate workspace:** `AGENTS_CONTEXT/ARCHITECTURE_STEWARD/SUBAGENTS/RUNTIME-CONSTITUTION-CORE-SUBSTRATE/`  
...
Read `AGENTS_CONTEXT/CORE_VS_PLUGIN_BOUNDARY/` in full, especially VISION, CANONICAL-MODEL, PASS-3 material and responsibility matrix; inspect Ω K0/K1 contracts/runtime, current invariants, plugin admission, Port transport, capability token, lifecycle, activation/recovery and crypto/platform seam evidence. Read Composition and Authority contexts to understand the boundaries this substrate must enforce without absorbing their semantics.
...
Steward the irreducible runtime guarantees required for any VIVIM composition to be safely admitted, isolated, invoked, revoked, activated, recovered and kept within its constitutional boundaries.
...
K0/K1 guarantees, admission, integrity/trust, isolation, transport, capability egress, revocation/fencing, lifecycle containment, atomic activation, recovery boundary, cryptographic primitives and minimal platform seam. Do not own product semantics, ontology, law policy, plugin semantics, provider behavior or general runtime orchestration.
...
What truly remains irreducible K0 rather than K1/plugin responsibility? Which fencing/state-arbitration guarantees are constitutional? Where should generic routing lookup live? What is the minimum platform seam? What evidence is required before promoting a mechanism into K0?
