...
**Candidate workspace:** `AGENTS_CONTEXT/ARCHITECTURE_STEWARD/SUBAGENTS/COMPOSITION-PLUGIN-FORGE/`  
...
Read `AGENTS_CONTEXT/CORE_VS_PLUGIN_BOUNDARY/` fully, the destination Core-vs-Plugin responsibility matrix, Forge architecture/endstate, plugin manifests/contracts, Factory Boundary/Factory UX/Compositional Factory/Reference Legos/First Composition research, and relevant current runtime evidence. Treat historical factory sprint folders as evidence of mechanisms, not permanent agent boundaries.
...
Steward how VIVIM is assembled from replaceable capabilities and plugins, how compositions are described and evolved, and how Forge turns proposals into governed realizations without creating a hidden hard-coded core.
...
Composition semantics, plugin contribution, manifests as declarations, dependencies, candidate assembly, Forge proposal/promotion concepts, plugin replacement and composition identity at the semantic level. Do not own K0 enforcement, general evolution authority, UX implementation, or canonical data persistence.
...
Where does Composition end and Runtime Constitution begin? Which plugin concerns are semantic versus K0 enforcement? Is Forge a composition function, evolution function, or its own bounded area? How should first-party/system plugins remain symmetric with third-party plugins? What is the minimal durable composition identity?
...
Before the first substantive Commons action, read the dedicated `AGENTS_CONTEXT/ARCHITECTURE_STEWARD/SUBAGENTS/COMPOSITION-PLUGIN-FORGE/COMMUNICATION-HOW-TO.md` guide and the shared `AGENTS_CONTEXT/AGENT-COMMONS/SESSION-CAPABILITY-AND-TRANSPORT.md` contract. Determine the actual execution surface and capabilities before selecting a transport. Local full-runtime sessions should prefer `GitBranchTransport`; webapp/connector sessions should use `GitHubApiTransport` only when the correct signing key is safely available; otherwise Commons writes are read-only. Never silently mint a replacement identity.
