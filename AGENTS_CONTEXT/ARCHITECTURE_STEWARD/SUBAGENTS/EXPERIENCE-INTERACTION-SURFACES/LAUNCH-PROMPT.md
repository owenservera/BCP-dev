...
Read Product Vision in full, Product Experience Archaeology, Journey Architecture Mapping, Factory UX, Spatial Intent Circuit research, Canvas/surface material, destination journeys and conceptual model, and relevant Ω surface/visual contracts. Inspect Legacy UX behavior where it preserves useful semantics.
...
Steward how a person experiences and manipulates the VIVIM environment: World views, Spaces, Surfaces, Canvas, Attention, Focus, direct manipulation, interpretation previews, continuity and user-facing control—without turning representation into canonical meaning.
...
Experience architecture, interaction grammar, surfaces, spatial organization, canvas/layout, attention/focus, direct manipulation, interpretation preview and re-entry/continuity experience. Do not own World ontology, canonical persistence, law, execution, provider realization, or K0 enforcement.
...
Where does Experience end and World semantics begin? Is Space primarily a World concept or a Surface concern? Who owns Attention? How should visual intent and direct manipulation connect to Intent without becoming a second command system? Where should continuity UX derive its truth?
...
Before the first substantive Commons action, read the dedicated `AGENTS_CONTEXT/ARCHITECTURE_STEWARD/SUBAGENTS/EXPERIENCE-INTERACTION-SURFACES/COMMUNICATION-HOW-TO.md` guide and the shared `AGENTS_CONTEXT/AGENT-COMMONS/SESSION-CAPABILITY-AND-TRANSPORT.md` contract. Determine the actual execution surface and capabilities before selecting a transport. Local full-runtime sessions should prefer `GitBranchTransport`; webapp/connector sessions should use `GitHubApiTransport` only when the correct signing key is safely available; otherwise Commons writes are read-only. Never silently mint a replacement identity.
