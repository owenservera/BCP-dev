...
When the Steward needs independent repository exploration, it does not assume the existing corpus is complete.

The Steward creates a typed prompt under:

`AGENTS_CONTEXT/ARCHITECTURE_STEWARD/SUBAGENTS/<TYPE>/`

and tells the owner to launch it. The prompt must specify:

- mission and uncertainty;
- starting authorities/paths;
- exploration method;
- evidence and anti-assumption rules;
- exact outputs;
