/**
 * VIVIM Plugin-Kernel — Contract Invariants (K-014)
 * @module plugin-kernel/core/invariants
 *
 ...
 *
 * Migrated from src/ai/core/invariants.ts (K0-15 subsystem). Logic is
 * unchanged; the import of types/hasCapability still points at the
 * pre-split K1 location because the types.ts SPLIT (K-012) has not yet
 * been executed. Once K-012 lands, the import path will become
 * '../core/types.js' (relative to this file).
 */

import type { AICapability, AIRequest, CapabilityMap, ProviderManifest } from '../../ai/core/types.js'
import { hasCapability } from '../../ai/core/types.js'
...
export function assertProviderManifest(provider: ProviderManifest): void {
  ...
  if (!provider.capabilities)
    throw new Error('Provider manifest must declare a capabilities map, even if empty.')
}
...
export function assertNotMutated<T extends object>(original: T, candidate: T, label: string): void {
  if (original !== candidate && JSON.stringify(original) !== JSON.stringify(candidate)) {
    throw new Error(
      `${label} was mutated across a protocol boundary. Construct a new value instead.`,
    )
  }
}
