// src/plugin-kernel/plugin-manager/install-router.ts
// K-003 row: the real install path (HTTP) moved to K0. Unification target for
// the K-001 (IPluginManager) + K-003 (this file) split per DESIGN.md D4.
//
...
//
// Migrated from src/server/plugin-router.ts. All src/ relative imports are
// rewritten to traverse up 3 levels to land at the project root (this file
// is at src/plugin-kernel/plugin-manager/, was at src/server/).
...
import { appErrorResponse, errorResponse, json } from '../../server/response.js'
