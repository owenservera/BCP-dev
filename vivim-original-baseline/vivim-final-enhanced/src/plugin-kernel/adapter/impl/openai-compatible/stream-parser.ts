/**
 * VIVIM Plugin-Kernel — OpenAI-Compatible SSE Stream Parser
 * @module plugin-kernel/adapter/impl/openai-compatible/stream-parser
 *
 * Parses the SSE stream from /v1/chat/completions (with stream: true) into
 * canonical AIEvent instances.
 *
 * Migrated from src/ai/protocol/openai-compatible/stream-parser.ts. Imports
 * kept at pre-split K1 paths; will collapse when K-012 (types.ts SPLIT) lands.
 * Local sibling (manifest) is now colocated.
 */

import type { AIEvent, ModelId, ProviderId, RequestId, ToolCallId } from '../../../../ai/core/types.js'
import { createEventId, toolCallId } from '../../../../ai/core/types.js'
