/**
 * VIVIM Plugin-Kernel — OpenAI-Compatible Adapter (K0-5 first-party impl)
 * @module plugin-kernel/adapter/impl/openai-compatible/adapter
 *
 ...
 *
 * Migrated from src/ai/protocol/openai-compatible/adapter.ts. The contract
 * (IProviderAdapter) still lives at src/ai/protocol/adapter.ts because
 * K-011 (P0-2 step 4 — contract SPLIT to src/plugin-kernel/adapter/contract.ts)
 * has not yet been executed. Once K-011 lands, the import path for the
 * contract will become '../../contract.js' (relative to this file).
 *
 * The 5 sibling files (auth.ts, error-mapper.ts, manifest.ts,
 * request-builder.ts, stream-parser.ts) are still being migrated in
 * subsequent PRs; the paths to them from this K0 location therefore
 * traverse up 4 levels and back into src/ai/protocol/openai-compatible/.
 * Once each sibling migrates to its K0 destination, the imports will
 * collapse to './<sibling>.js'.
 */
...
import { modelId, VIVIM_AI_PROTOCOL } from '../../../../ai/core/types.js'
import type { IProviderAdapter, ProviderConnection } from '../../../../ai/protocol/adapter.js'
import { resolveAuthHeaders } from '../../../../ai/protocol/openai-compatible/auth.js'
...
import { buildChatCompletionRequest } from '../../../../ai/protocol/openai-compatible/request-builder.js'
...
export class OpenAICompatibleAdapter implements IProviderAdapter {
  ...
  private resolveModelId(request: AIRequest): import('../../../../ai/core/types.js').ModelId {
