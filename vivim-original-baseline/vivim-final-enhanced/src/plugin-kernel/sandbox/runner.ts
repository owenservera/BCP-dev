/**
 * VIVIM Plugin-Kernel — Sandbox Runner (K-027)
 * @module plugin-kernel/sandbox/runner
 *
 * QuickJS WASM-based sandbox. The only production path. No vm fallback in
 * K0 (H9 hazard remediation; the deprecated vm impl in
 * `src/engines/sandbox-runner-vm.ts` will be removed by a subsequent PR).
 *
 * Migrated from src/engines/sandbox-runner.ts (selector) and
 * src/engines/sandbox-runner-quickjs.ts (impl) per the MERGE verdict.
 *
 * Public API:
 *   - Interfaces: SandboxPermissions, SandboxBudget, SandboxRunOptions, SandboxResult
 *   - Class: SandboxRunner (the class itself, not a selector)
 *
 * True process-level isolation via WASM:
 * - No shared heap with the host
 * - No access to process/require/globalThis
 * - Hard memory limit per execution
 *
 * Parser function calls are bridged back to the host via a context
 * registry keyed by handlerSlug, with LRU eviction at MAX_CTX entries.
 */
...
// ── QuickJS WASM backend (the only production path) ───────────────────────
