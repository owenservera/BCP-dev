...
> order line below is void in full, Chrome-only is the order.
...
**Order:** Chrome master/slave first — browser surfaces by fixture availability, never by live-network convenience. *(D-456: the Ollama-first order is gone from the plan; the banner above records the void.)*
