...
- **Objective:** every legacy provider (T-01/T-02/T-03) as a realization row + handler: Chrome master/slave first — BROWSER_MEDIATED throughout (ChatGPT/Claude/Gemini + remainder), no AI-API realization anywhere in the wave (D-456: the local-model pilot ordering is gone from the plan), launch mode (Upgrade 4) + stealth admission (T-07 file-by-file) + governor restructure (T-08) behind the D-338 bar.
- **Compositions:** `browser.json` extended via generator; proving compositions stay untagged behind the D-420 fence.
