...
lineage `D-449`, falsifiers green before the flip): the self-hosting proof
is now a system capability, not a team ceremony. The ledger bundle IS the
frozen archive; a five-stage chain of custody (resolve → manifest walk →
toolchain → hermeticity → gate digest) proves it on demand, headless, with
every stage hash-pinned and every unprovable stage honestly ADVISORY;
`boot.emit` re-proves the Forge byte-identical to itself by root-hash
comparison (core falsifier F2, mechanical). No gate stage was added (the
boot chain is its own ceremony; the genome layer law carries Ω-0's
evidence kind). Pass #6 below stands unchanged.
...
## The hermetic ground (Ω-0, D-431 — spec lineage `D-449`)

- **The archive is the bundle** — no second format, no privileged path: the
  ledger's latest `_N.bundle` (resolved through the `D-422` A17 pin) is the
  frozen archive; its sha256 is pinned truncated in the generated README
  row (cross-checked every verify) and FULL-strength in the boot store's
  archive row from first verify onward.
- **The five-stage chain of custody** (`omega:boot verify`, `tooling/gates/
  boot.ts`): S1 resolve → S2 manifest walk → S3 toolchain → S4 hermeticity →
  S5 gate digest (`--gate`; the booted tree's OWN gate, offline). Each stage
  a chained `boot/` row with inputHash/outputHash and verdict `verified |
  advisory | refused`; a refused stage HALTS (nothing after it runs, no
  receipt); a receipt exists only when nothing refused, and its badge is
  VERIFIED only when every stage verified — the `D-321` honesty contract:
  unproven is ADVISORY, never silently VERIFIED.
- **No unlisted file executes:** unlisted guests are exactly what git
  reports untracked AND not ignored — git's own ignore law draws the line
  between the system (tracked) and declared local memory (`boot/`,
  `sessions/`, `dev-vault/`, the `D-428` boundary). The verifier at
  `tooling/gates/boot.ts` must hash-match the manifest — the tool proving
  the ground stands on it or refuses.
- **The mechanical self-forge (F2):** `omega:boot emit --consent` (consent
  required — `BOOT_NO_CONSENT`; a prior VERIFIED receipt required —
  `BOOT_NO_RECEIPT`) re-walks the running tree's manifest against the
  pinned archive's, root-hash to root-hash; one byte off is
  `BOOT_EMIT_DIVERGENCE` naming the path.
- **The refusal register** (each a sentence, each ledgered, each
  replayable via `omega:boot audit`): `BOOT_ARCHIVE_HASH_MISMATCH`,
  `BOOT_UNVERIFIED_FILE`, `BOOT_TOOLCHAIN_DRIFT`, `BOOT_NETWORK_VIOLATION`,
  `BOOT_EMIT_DIVERGENCE`, `BOOT_NO_RECEIPT`, `BOOT_NO_CONSENT` (the last
  the tree-idiom addition for emit consent). Network markers are legal
  ONLY under `surfaces/` + `plugins/` — the post-boot canvas/providers,
  the headless doctrine.
- `boot/` is environment-local (gitignored; law committed, memory local —
  the `D-428` boundary). Tamper-EVIDENT, not tamper-PROOF: audit re-derives
  every digest and names the lie.
...
- **The Chrome-only plan (D-456, RATIFIED):** the Ollama concept leaves every planning doc — vision §3 list plus Frank, the W2 wave map, the strategy strata, the triage rows — with D-456 markers; Chrome-mediated is the only named provider path. RATIFIED records, round and audit logs, history rows, and the fenced `provider.llm` proving path stand as cited history, never as plan.
...
- **Tests: 1253 (the D-431 tree; 1247 + 6 F-BOOT), sharding trigger long crossed** — the quick gate + lanes hold it; re-check wall-time as the suite grows past ~1,300.
