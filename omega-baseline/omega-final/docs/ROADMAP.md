...
> **Extended by D-456 (RATIFIED):** the Ollama concept leaves the plan — the
> W2 sequencing below names Chrome-only; any surviving pilot-first line is void.
...
**Goal:** prove "providers as graded realizations" on the Chrome substrate — BROWSER_MEDIATED throughout, Chrome master/slave first by fixture availability, never live-network convenience. (D-456: the API_NATIVE-pilot ordering is gone from the plan; the banner above records the void.)

- **W2-a · Chrome-mediated pilot (least-defended browser surface per import-parser fixtures).** Realization
