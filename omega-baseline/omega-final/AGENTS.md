...
- **Hermetic bootstrap (D-431, Ω-0):** the ledger bundle IS the frozen
  archive; `bun run omega:boot verify --gate` re-proves it on demand —
  S1 archive hash → S2 manifest walk (no unlisted file executes; git's own
  ignore law draws the local-memory line) → S3 toolchain pin → S4
  hermeticity (zero network reach in the bootstrap path; markers are legal
  only under `surfaces/` + `plugins/`) → S5 the booted tree's OWN gate,
  offline. Every stage is hash-pinned in the chained `boot/` store; a
  refused stage HALTS; unprovable is ADVISORY, never silently VERIFIED
  (the D-321 contract). `omega:boot emit --consent` re-proves the Forge
  byte-identical to its own archive (F2, mechanical) — it requires a prior
  VERIFIED receipt and names every divergent path. `omega:boot audit`
  replays the chain. At wave close, after `omega:round-close` cuts the
  bundle: run `omega:boot verify --gate` + `emit --consent` against it —
  the self-hosting boot dogfooded on its own landing wave.
